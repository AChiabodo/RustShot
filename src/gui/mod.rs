...
impl RustShot {
        ...
        CentralPanel::default().show(ctx, |ui| match self.screenshot.clone() {
            ...
            Some(screenshot) => {
                ScrollArea::both().show(ui, |ui| {
                    ...
                    let img = ui.add(ImageButton::new(retained_img.texture_id(ctx), retained_img.size_vec2()));
    ...
    fn paint_logic(&mut self, img: Response) {
        if img.clicked() {
            ...
            self.screenshot.as_mut().unwrap().draw_pixel(self.paint_info.curr_ptr.x as u32, self.paint_info.curr_ptr.y as u32, Rgba([0u8, 0u8, 0u8, 255u8]));
            //Draw 15x15 circle around the pointer
            let white = Rgb([255u8, 255u8, 255u8]);
            let new_screen: Image<Rgb<u8>> = drawing::draw_filled_circle(self.screenshot.as_ref().unwrap().as_rgb8().unwrap(),
                                                                          (self.paint_info.curr_ptr.x as i32, self.paint_info.curr_ptr.y as i32), 5, white);
            self.screenshot = Some(DynamicImage::from(new_screen));
        } else {
